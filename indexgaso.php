...
if(isset($_POST['search']))
{
	...
	$query = "SELECT * FROM `membergaso` WHERE  CONCAT(`trackid`)LIKE '%".$valueToSearch."%'";
  //  $query = "SELECT * FROM `membertracking` WHERE  CONCAT(`mem_id`, `trackid`, `firstname`, `lastname`, `address`)LIKE '%".$valueToSearch."%'";
    $search_result = filterTable($query);
    
}
    else {
        $query ="SELECT * FROM `membergaso` ORDER BY mem_id DESC";
        $search_result = filterTable($query);
...
<div class="container">
  ...
  <!--  <ul class="nav navbar-nav">
    ...
    <li> <a href="chathrmo.php"><font color="White" size="4"><B>Chat Room</B></font></a></li>
...
<div class="col-md-3"></div>
	...
	<center><h3 class="text-primary"><B>Office of the General Administrative Services </B></h3></center>
...
<center><a href="del_hrmo.php?mem_id=<?php //echo $fetch['mem_id']?>"><i class="fa fa-trash" style='font-size:16px;color:Blue'><font size="4"><B>Del</B></font></i></a></center>
...
function sds(data_id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location = ("transferhrmo_sds.php?mem_id=" + data_id);   
