<?php
	...
	if(ISSET($_REQUEST['mem_id'])){
		...
		$query=mysqli_query($conn, "SELECT * FROM `membersgod` WHERE `mem_id`='$mem_id'") or die(mysqli_error());
		...
		mysqli_query($conn, "DELETE FROM `membersgod` WHERE `mem_id`='$mem_id'") or die(mysqli_error());
		
		// echo"<script>confirm('Data successfully transfer')</script>";
		echo"<script>window.location='indexsgod.php'</script>";
