<?php
	...
	if(ISSET($_REQUEST['mem_id'])){
		...
		$query=mysqli_query($conn, "SELECT * FROM `memberdpsu` WHERE `mem_id`='$mem_id'") or die(mysqli_error());
		...
		mysqli_query($conn, "DELETE FROM `memberdpsu` WHERE `mem_id`='$mem_id'") or die(mysqli_error());
		
		echo"<script>alert('Data successfully transfer')</script>";
		echo"<script>window.location='indexdpsu.php'</script>";
