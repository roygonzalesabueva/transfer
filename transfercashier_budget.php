<?php
	...
	if(ISSET($_REQUEST['mem_id'])){
		...
		$query=mysqli_query($conn, "SELECT * FROM `membercashier` WHERE `mem_id`='$mem_id'") or die(mysqli_error());
		...
		mysqli_query($conn, "INSERT INTO `memberbudget` (mem_id,trackid,firstname,lastname,section,address) VALUES('','$trackid','$firstname', '$lastname', '$section','$address')") or die(mysqli_error());
		...
		mysqli_query($conn, "DELETE FROM `membercashier` WHERE `mem_id`='$mem_id'") or die(mysqli_error());
		
		// echo"<script>confirm('Data successfully transfer')</script>";
		echo"<script>window.location='indexcashier.php'</script>";
		header("refresh:1; url=indexcashier.php");
		header("refresh:1; url=indexbudget.php");
	}
?>